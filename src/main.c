...
void
parse_args (unsigned int argc, char **argv, opts_t *mbf_opts)
{
...
bool
read_file_to_dstr (const char *path, dstr_t *out)
{
...
int
write_to_file (const char *path, const char *content)
{
...
run_mbf (opts_t *opts)
{
   ...
   if (!opts->skip_expansion)
      {
         expanded    = mbf_preprocess (code_to_run);
